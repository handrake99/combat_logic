...
namespace SteamB23.KoreanUtility.Grammar
{
    ...
    public static class 조사
    {
        ...
        /// <param name="textBuilder">대상 <c>StringBuilder</c>입니다.</param>
        /// <param name="type">추가될 조사의 타입입니다.</param
        /// <param name="surfix">단어뒤 조사앞에 추가로 붙이고 싶은 문자열입니다.</param>
        public static void 조사추가(this StringBuilder textBuilder, 조사종류 type, string surfix)
        {
            string text = textBuilder.ToString();
        ...
        {
            정규식컴파일();
            string[] rawSpritedText = josaSignRegex.Split(text);
            char[] removal = { '[', '-', ']' };

            // Tweak
            // 크로매틱소울의 컬러 종료 심볼([-])에 대한 조치를 추가함.
            string[] spritedText = new string[rawSpritedText.Length];
            string[] colorSymbols = new string[rawSpritedText.Length];

            for (var i = 0; i < rawSpritedText.Length; i++)
            {
                if(rawSpritedText[i].EndsWith("[-]"))
                {
                    spritedText[i] = rawSpritedText[i].TrimEnd(removal);
                    colorSymbols[i] = "[-]";
                }
                else
                {
                    spritedText[i] = rawSpritedText[i];
                }
            }
            ...
            for (int i = 0; i < matchedText.Count; i++)
            {
                switch (matchedText[i].Value)
                {
                    ...
                    case "[는은]":
                        ...
                        resultText.조사추가(조사종류.은는, colorSymbols[i]);
                        break;
                    case "[이가]":
                    case "[가이]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.이가, colorSymbols[i]);
                        break;
                    case "[을를]":
                    case "[를을]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.을를, colorSymbols[i]);
                        break;
                    case "[과와]":
                    case "[와과]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.과와, colorSymbols[i]);
                        break;
                    case "[아야]":
                    case "[야아]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.아야, colorSymbols[i]);
                        break;
                    case "[으로]":
                    case "[로]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.으로, colorSymbols[i]);
                        break;
                    case "[이]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.이, colorSymbols[i]);
                        break;
                    case "[이에]":
                    case "[예]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.이에, colorSymbols[i]);
                        break;
                    case "[이어]":
                    case "[여]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.이어, colorSymbols[i]);
                        break;
                    case "[이었]":
                    case "[였]":
                        resultText.Append(spritedText[i]);
                        resultText.조사추가(조사종류.이었, colorSymbols[i]);
                        break;
                }
            }
